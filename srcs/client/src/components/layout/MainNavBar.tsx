...
function MainNavBar(props: any) {
    ...
    function acceptGame(data: string[]) {
      ...
      setTimeout(() => {socket.emit('req_splice', data[1])}, 500);
    }

    function rejectGame(data: string) {
      socket.emit('req_splice', data);
    }
    ...
    function notifyGame(data: string[]) {
      const Msg = () => (
        ...
        <svg onClick={() => rejectGame(data[1])} color="red" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-x-circle" viewBox="0 0 16 16">
        <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
        ...
        if (data) {
          if (data[0] === 'friendrequest') {
            ...
            notifyGame(data);
              ...
              if (user.currentChannel !== user.username+"&"+data[1] && user.currentChannel !== data[1]+"&"+user.username)
              {
                  setNotifications(true);
          ...
          else if (data[0] === 'reject_game_request') {
              Utils.notifyErr('You already sent a game request');
          }
